      ...
      <div className="container mx-auto px-10">
          ...
          {anchors.map((a, i) => (
            <button key={i} onClick={() => setActiveAnchor(i)} className="text-left"
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: "1rem" }}>
                <div style={{ width: 32, height: 32, borderRadius: "0.5rem", backgroundColor: activeAnchor === i ? a.color : "#f0ece6", color: activeAnchor === i ? "#fff" : "#4a5568", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem", fontWeight: 800, flexShrink: 0, transition: "all 0.2s" }}>
                  {a.number}
                </div>
                <div>
                  <div style={{ fontSize: "1.0625rem", fontWeight: 700, color: "#1a1a2e", marginBottom: "0.625rem", letterSpacing: "-0.01em" }}>{a.title}</div>
                  <p style={{ fontSize: "1rem", color: "#4a5568", lineHeight: 1.75 }}>{a.body}</p>
                  {activeAnchor === i && (
                    <div style={{ marginTop: "1rem", paddingLeft: "0.875rem", borderLeft: `2px solid ${a.color}` }}>
                      <p style={{ fontSize: "1.0625rem", fontStyle: "italic", fontWeight: 500, color: a.color }}>"{a.quote}"</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="grid md:grid-cols-2" style={{ gap: "3rem", alignItems: "start", marginBottom: "3.5rem" }}>
          <div>
            ...
            <p style={{ fontSize: "1rem", color: "#3d4759", lineHeight: 1.75, marginBottom: "1.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {[
                ...
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem" }}>
                  ...
                  <div>
                    <span style={{ fontSize: "1rem", fontWeight: 600, color: "#1a1a2e" }}>{item.title} — </span>
                    <span style={{ fontSize: "1rem", color: "#3d4759" }}>{item.body}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ border: "1px solid #dcd6ce", borderRadius: "0.875rem", overflow: "hidden" }}>
            <div style={{ padding: "0.875rem 1.25rem", backgroundColor: "#f0ece6", borderBottom: "1px solid #dcd6ce" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.5rem" }}>
                <span style={{ fontSize: "1rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#3d4759" }}>Capability</span>
                <span style={{ fontSize: "1rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "#9ca3af", textAlign: "center" }}>Legacy IAM</span>
                <span style={{ fontSize: "1rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#00297a", textAlign: "center" }}>Unkov</span>
              </div>
            </div>
            {rows.map((row, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.5rem", padding: "0.75rem 1.25rem", borderBottom: i < rows.length - 1 ? "1px solid #e5e7eb" : "none", backgroundColor: i % 2 === 0 ? "#ffffff" : "#faf9f7" }}>
                <span style={{ fontSize: "1.0625rem", fontWeight: 500, color: "#3d3d5c" }}>{row.cap}</span>
                <span style={{ fontSize: "1.0625rem", color: "#9ca3af", textAlign: "center" }}>{row.legacy}</span>
                <span style={{ fontSize: "1.0625rem", fontWeight: 600, color: "#00297a", textAlign: "center" }}>{row.unkov}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: "3.5rem" }}>
          <button onClick={() => setShowCompetitors(!showCompetitors)}
            style={{ fontSize: "1rem", fontWeight: 600, color: "#0061d4", background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex", alignItems: "center", gap: "0.375rem", marginBottom: showCompetitors ? "1.25rem" : 0 }}>
            <span style={{ fontSize: "1rem" }}>{showCompetitors ? "▲" : "▼"}</span> How Unkov beats named competitors
          </button>
          {showCompetitors && (
            <div style={{ border: "1px solid #dcd6ce", borderRadius: "0.875rem", overflow: "hidden" }}>
              <div style={{ padding: "0.875rem 1.25rem", backgroundColor: "#f0ece6", borderBottom: "1px solid #dcd6ce", display: "grid", gridTemplateColumns: "1fr 1fr 1.25fr", gap: "0.5rem" }}>
                {["Competitor", "Their Moat", "Unkov's Advantage"].map((h, i) => (
                  <span key={i} style={{ fontSize: "1rem", fontWeight: i === 2 ? 700 : 600, textTransform: "uppercase", letterSpacing: "0.08em", color: i === 2 ? "#00297a" : "#3d4759" }}>{h}</span>
                ))}
              </div>
              {competitors.map((c, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1.25fr", gap: "0.5rem", padding: "0.75rem 1.25rem", borderBottom: i < competitors.length - 1 ? "1px solid #e5e7eb" : "none", backgroundColor: i % 2 === 0 ? "#ffffff" : "#faf9f7" }}>
                  <span style={{ fontSize: "1.0625rem", fontWeight: 700, color: "#1a1a2e" }}>{c.name}</span>
                  <span style={{ fontSize: "1.0625rem", color: "#6b7280" }}>{c.moat}</span>
                  <span style={{ fontSize: "1.0625rem", fontWeight: 600, color: "#00297a" }}>{c.unkovWin}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ background: "linear-gradient(135deg, #00297a 0%, #0041a8 100%)", borderRadius: "1rem", padding: "2.5rem", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "1rem", color: "rgba(255,255,255,0.65)", maxWidth: "520px", margin: "0 auto", lineHeight: 1.7 }}>

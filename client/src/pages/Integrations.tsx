...
const CONNECTOR_CONFIG: Record<string, {
  ...
  "google-workspace": {
    ...
    logoBg: "#4285f4",
    logoColor: "#fff",
    tagline: "Directory sync — users, groups, shared drives, service accounts.",
    setupTime: "10 min",
    what: [
      "All Google Workspace users and their roles",
      "Groups and group memberships",
      "Service accounts and their key ages",
      "Suspended accounts still assigned to groups",
      "Admin-role assignments (Super Admin, Groups Admin)",
    ],
    finds: [
      { sev: "critical", label: "Super Admin account inactive 60+ days" },
      { sev: "critical", label: "Service account key 180+ days old" },
      { sev: "high",     label: "Suspended user still in privileged group" },
      { sev: "high",     label: "Service account with domain-wide delegation" },
    ...
    envKeys: ["GOOGLE_SA_KEY_JSON", "GOOGLE_WORKSPACE_DOMAIN"],
    commands: ["npm run seed:google-workspace", "npm run google-workspace:scan", "npm run db:write"],
    docsPath: "/docs#google-workspace",
  },
  "ping-identity": {
    id: "ping-identity",
    name: "Ping Identity",
    ...
    logoBg: "#E1001A",
    logoColor: "#fff",
    tagline: "OAuth 2.0 / OIDC federation — users and federated identities.",
    setupTime: "10 min",
    what: [
      "All PingOne users and population memberships",
      "Active application assignments",
      "Federated identity connections (SAML, OIDC)",
      "MFA enrollment status per user",
      "Admin and operator role grants",
    ],
    finds: [
      { sev: "critical", label: "Admin account without MFA enrolled" },
      { sev: "high",     label: "Federated identity with excessive app scope" },
      { sev: "high",     label: "Dormant user with active app assignments (90+ days)" },
      { sev: "medium",   label: "Application with no users assigned (orphaned)" },
    ],
    fields: [
      { key: "PING_ENV_ID",     label: "Environment ID",  placeholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
        hint: "PingOne Console → Settings → Environment → Environment ID" },
      { key: "PING_CLIENT_ID",  label: "Client ID",       placeholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
        hint: "PingOne Console → Applications → Add Application → Worker → Client ID" },
      { key: "PING_CLIENT_SECRET", label: "Client Secret", placeholder: "...", secret: true,
        hint: "Same Worker app → Configuration tab → Client Secret (needs Environment Admin read scope)" },
    ],
    envKeys: ["PING_ENV_ID", "PING_CLIENT_ID", "PING_CLIENT_SECRET"],
    commands: ["npm run seed:ping", "npm run ping:scan", "npm run db:write"],
    docsPath: "/docs#ping-identity",
  },
  "jumpcloud": {
    id: "jumpcloud",
    name: "JumpCloud",
    ...
    logoBg: "#6CC04A",
    logoColor: "#fff",
    tagline: "SCIM 2.0 provisioning — users, groups, device identities.",
    setupTime: "5 min",
    what: [
      "All JumpCloud users and their system bindings",
      "Group memberships and application associations",
      "Device-bound identities (MDM-enrolled machines)",
      "Admin accounts and permission levels",
      "LDAP-bound users",
    ],
    finds: [
      { sev: "critical", label: "Admin account bound to unmanaged device" },
      { sev: "high",     label: "User with system access, no group membership (shadow account)" },
      { sev: "high",     label: "Inactive user account (60+ days) with active app bindings" },
      { sev: "medium",   label: "LDAP user without MFA" },
    ],
    fields: [
      { key: "JUMPCLOUD_API_KEY", label: "API Key", placeholder: "...", secret: true,
        hint: "JumpCloud Admin Portal → top-right profile icon → API Settings → API Key (read-only admin key)" },
    ],
    envKeys: ["JUMPCLOUD_API_KEY"],
    commands: ["npm run seed:jumpcloud", "npm run jumpcloud:scan", "npm run db:write"],
    docsPath: "/docs#jumpcloud",
  },
  "google-cloud-iam": {
    id: "google-cloud-iam",
    ...
    logoBg: "#34a853",
    logoColor: "#fff",
    tagline: "Service account governance, workload identity, IAM policy drift.",
    setupTime: "10 min",
    what: [
      "All IAM service accounts per project",
      "Service account key ages (rotation enforcement)",
      "Workload identity federation bindings",
      "IAM policy bindings at project, folder, org level",
      "Primitive roles (Owner, Editor) — high-risk grants",
    ],
    finds: [
      { sev: "critical", label: "Service account key 90+ days old" },
      { sev: "critical", label: "Primitive Owner role granted to external user" },
      { sev: "high",     label: "Service account with project-level Editor" },
      { sev: "high",     label: "Workload identity binding to unknown pool" },
    ...
    envKeys: ["GCP_SA_KEY_JSON", "GCP_PROJECT_ID"],
    commands: ["npm run seed:gcp", "npm run gcp:scan", "npm run db:write"],
    docsPath: "/docs#google-cloud-iam",
  },
  "adp": {
    id: "adp",
    name: "ADP",
    ...
    logoBg: "#D40000",
    logoColor: "#fff",
    tagline: "Payroll-driven deprovisioning — termination events trigger immediate access revocation.",
    setupTime: "15 min",
    what: [
      "Active employees (name, email, department, status)",
      "Termination and leave-of-absence events",
      "New hire events (trigger provisioning workflows)",
      "Role changes (department transfers, promotions)",
      "Cross-reference with IdP to surface ghost accounts",
    ],
    finds: [
      { sev: "critical", label: "Terminated employee with active IdP account" },
      { sev: "critical", label: "Terminated employee with active cloud credentials" },
      { sev: "high",     label: "Employee on leave with full access entitlements" },
      { sev: "high",     label: "Department transfer — access not reviewed within 7 days" },
    ],
    fields: [
      { key: "ADP_CLIENT_ID",     label: "Client ID",     placeholder: "...",
        hint: "ADP Marketplace → API Credentials → Client ID for your registered app" },
      { key: "ADP_CLIENT_SECRET", label: "Client Secret", placeholder: "...", secret: true,
        hint: "ADP Marketplace → API Credentials → Client Secret (requires HR Practitioner API scope)" },
      { key: "ADP_CERT_PEM",      label: "Client Certificate (PEM)", placeholder: "-----BEGIN CERTIFICATE-----...", secret: true,
        hint: "ADP requires mutual TLS — upload the PEM cert from your registered application" },
    ],
    envKeys: ["ADP_CLIENT_ID", "ADP_CLIENT_SECRET", "ADP_CERT_PEM"],
    commands: ["npm run seed:adp", "npm run adp:scan", "npm run db:write"],
    docsPath: "/docs#adp",
  },
  "gitlab": {
    id: "gitlab",
    ...
    logoBg: "#FC6D26",
    logoColor: "#fff",
    tagline: "Group members, deploy tokens, CI/CD variables with secrets access.",
    setupTime: "5 min",
    what: [
      "All group members and their access levels",
      "Deploy tokens and their expiry dates",
      "CI/CD variable scopes (project vs group vs instance)",
      "Protected branches and merge request approval rules",
      "Personal access tokens with wide scope",
    ],
    finds: [
      { sev: "critical", label: "Deploy token with write access — no expiry set" },
      { sev: "critical", label: "CI/CD variable with secret value — unmasked" },
      { sev: "high",     label: "Group Owner with no MFA" },
      { sev: "high",     label: "Personal access token with api scope — 90+ days old" },
      { sev: "medium",   label: "Guest user with access to protected branch" },
    ],
    fields: [
      { key: "GITLAB_TOKEN",    label: "Personal Access Token", placeholder: "glpat-...", secret: true,
        hint: "GitLab → Profile → Access Tokens → Create (read_api, read_user, read_repository scopes)" },
      { key: "GITLAB_GROUP_ID", label: "Top-level Group ID",    placeholder: "12345678",
        hint: "Your top-level GitLab group → Settings → General → Group ID (shown at top)" },
    ],
    envKeys: ["GITLAB_TOKEN", "GITLAB_GROUP_ID"],
    commands: ["npm run seed:gitlab", "npm run gitlab:scan", "npm run db:write"],
    docsPath: "/docs#gitlab",
  },
  "hashicorp-vault": {
    id: "hashicorp-vault",
    ...
    logoBg: "#1563FF",
    logoColor: "#fff",
    tagline: "Secrets lifecycle governance — dynamic credentials, lease expiry tracking.",
    setupTime: "10 min",
    what: [
      "Active leases and TTL remaining per secret engine",
      "Token accessor inventory (orphaned token detection)",
      "Auth method mounts and their configurations",
      "Policy assignments per entity",
      "Dynamic credential generation patterns (AWS, DB)",
    ],
    finds: [
      { sev: "critical", label: "Root token in use (should never be used operationally)" },
      { sev: "critical", label: "Secret lease with no TTL (non-expiring credential)" },
      { sev: "high",     label: "Orphaned token accessor (parent token revoked)" },
      { sev: "high",     label: "Entity with wildcard policy access" },
      { sev: "medium",   label: "Auth mount with default TTL > 24 hours" },
    ],
    fields: [
      { key: "VAULT_ADDR",  label: "Vault Address",  placeholder: "https://vault.yourdomain.com:8200",
        hint: "The full URL of your Vault cluster including port" },
      { key: "VAULT_TOKEN", label: "Vault Token",    placeholder: "hvs.xxxxxxxxxxxx", secret: true,
        hint: "Create a policy-scoped token with sys/leases/lookup, auth/token/accessors, sys/mounts, and identity/* read access" },
    ],
    envKeys: ["VAULT_ADDR", "VAULT_TOKEN"],
    commands: ["npm run seed:vault", "npm run vault:scan", "npm run db:write"],
    docsPath: "/docs#hashicorp-vault",
  },
  "github-actions": {
    id: "github-actions",
    name: "GitHub Actions",
    ...
    logoBg: "#2088FF",
    logoColor: "#fff",
    tagline: "Workflow permissions, OIDC tokens, environment secrets access.",
    setupTime: "5 min",
    what: [
      "Workflow files and their declared permissions",
      "Environments and their secret + variable bindings",
      "OIDC token claims and cloud trust configurations",
      "Workflows with write access to sensitive resources",
      "Scheduled workflows — potential long-lived attack surface",
    ],
    finds: [
      { sev: "critical", label: "Workflow with write: all permissions (overly broad)" },
      { sev: "critical", label: "OIDC token trusting wildcard branch pattern" },
      { sev: "high",     label: "Production environment with no reviewer protection" },
      { sev: "high",     label: "Secret accessible from feature branches" },
      { sev: "medium",   label: "Scheduled workflow running with stale token" },
    ],
    fields: [
      { key: "GITHUB_TOKEN",    label: "Personal Access Token", placeholder: "ghp_...", secret: true,
        hint: "github.com/settings/tokens → Generate token with read:org, repo, workflow scopes" },
      { key: "GITHUB_ORG",      label: "Organization name",    placeholder: "your-org",
        hint: "Your GitHub organization name (github.com/{org})" },
    ],
    envKeys: ["GITHUB_TOKEN", "GITHUB_ORG"],
    commands: ["npm run seed:github-actions", "npm run github-actions:scan", "npm run db:write"],
    docsPath: "/docs#github-actions",
  },
  "splunk": {
    id: "splunk",
    name: "Splunk",
    ...
    logoBg: "#000000",
    logoColor: "#00A4EF",
    tagline: "Real-time identity event streaming — Identity Drift alerts as Splunk events.",
    setupTime: "10 min",
    what: [
      "Splunk user accounts and role assignments",
      "Search head cluster user activity",
      "Index-level access controls (who can query what)",
      "Saved search + alert ownership",
      "Authentication log anomalies",
    ],
    finds: [
      { sev: "critical", label: "Admin account with no activity 90+ days" },
      { sev: "high",     label: "User with access to sensitive index (PII, PCI)" },
      { sev: "high",     label: "Shared service account used for scheduled searches" },
      { sev: "medium",   label: "User with sc_admin role — over-privileged for analyst" },
    ],
    fields: [
      { key: "SPLUNK_HOST",     label: "Splunk Host",  placeholder: "https://splunk.yourdomain.com:8089",
        hint: "Your Splunk management API URL (port 8089 by default)" },
      { key: "SPLUNK_USERNAME", label: "Username",     placeholder: "unkov-scanner",
        hint: "Create a dedicated service account with admin read access (not admin write)" },
      { key: "SPLUNK_PASSWORD", label: "Password",     placeholder: "...", secret: true,
        hint: "Password for the scanner account — or use a session token" },
    ],
    envKeys: ["SPLUNK_HOST", "SPLUNK_USERNAME", "SPLUNK_PASSWORD"],
    commands: ["npm run seed:splunk", "npm run splunk:scan", "npm run db:write"],
    docsPath: "/docs#splunk",
  },
  "1password": {
    id: "1password",
    ...
    logoBg: "#1A8CFF",
    logoColor: "#fff",
    tagline: "Team vault governance — shared items, stale credentials, access scope.",
    setupTime: "5 min",
    what: [
      "All vaults and their membership assignments",
      "Shared credentials (detected by duplicate usage)",
      "Stale items — passwords not rotated in 90+ days",
      "Guest accounts with vault access",
      "Permissions per vault per group",
    ],
    finds: [
      { sev: "critical", label: "Shared login item used by 5+ employees (no individual accountability)" },
      { sev: "high",     label: "Production credential not rotated in 180+ days" },
      { sev: "high",     label: "Guest account with access to Production vault" },
      { sev: "medium",   label: "Personal vault item tagged as shared — no audit trail" },
    ],
    fields: [
      { key: "OP_SERVICE_ACCOUNT_TOKEN", label: "Service Account Token", placeholder: "ops_...", secret: true,
        hint: "1Password.com → Developer Tools → Service Accounts → Create. Grant read access to all vaults you want audited." },
    ],
    envKeys: ["OP_SERVICE_ACCOUNT_TOKEN"],
    commands: ["npm run seed:1password", "npm run 1password:scan", "npm run db:write"],
    docsPath: "/docs#1password",
  },
  "servicenow": {
    id: "servicenow",
    name: "ServiceNow",
    ...
    logoBg: "#62D84E",
    logoColor: "#fff",
    tagline: "Auto-create remediation tickets — access reviews, CMDB enrichment.",
    setupTime: "10 min",
    what: [
      "ServiceNow user accounts and role assignments",
      "CMDB configuration item ownership",
      "Open access request tickets and approvers",
      "Privileged role grants (admin, security_admin)",
      "User accounts mapped to CMDB identities",
    ],
    finds: [
      { sev: "critical", label: "security_admin role with no MFA" },
      { sev: "high",     label: "User with admin role inactive 60+ days" },
      { sev: "high",     label: "CMDB CI with no identified owner" },
      { sev: "medium",   label: "Open access request older than 30 days (stuck in queue)" },
    ],
    fields: [
      { key: "SNOW_INSTANCE", label: "Instance URL", placeholder: "https://your-instance.service-now.com",
        hint: "Your ServiceNow instance URL — found in your browser when logged in" },
      { key: "SNOW_USERNAME", label: "Username",     placeholder: "unkov.scanner",
        hint: "Create a dedicated user with itil and asset roles — no admin required" },
      { key: "SNOW_PASSWORD", label: "Password",     placeholder: "...", secret: true,
        hint: "Password for the scanner account" },
    ],
    envKeys: ["SNOW_INSTANCE", "SNOW_USERNAME", "SNOW_PASSWORD"],
    commands: ["npm run seed:servicenow", "npm run servicenow:scan", "npm run db:write"],
    docsPath: "/docs#servicenow",
  },
  "jira": {
    id: "jira",
    ...
    logoBg: "#0052CC",
    logoColor: "#fff",
    tagline: "Access workflow automation — violation findings create Jira issues automatically.",
    setupTime: "5 min",
    what: [
      "Jira users and project role memberships",
      "Service management agent assignments",
      "Project admin and global admin role holders",
      "Open access review issues and SLA compliance",
      "Automation rule triggers tied to identity events",
    ],
    finds: [
      { sev: "critical", label: "Project admin with no activity 90+ days" },
      { sev: "high",     label: "External user with internal project admin role" },
      { sev: "high",     label: "Access review issue breached SLA (overdue >14 days)" },
      { sev: "medium",   label: "Service account used for automation — wide project scope" },
    ],
    fields: [
      { key: "JIRA_BASE_URL",  label: "Jira Base URL", placeholder: "https://your-org.atlassian.net",
        hint: "Your Atlassian Cloud URL" },
      { key: "JIRA_EMAIL",     label: "Account Email", placeholder: "unkov-scanner@yourorg.com",
        hint: "The email address of a dedicated service account with Browse Users and Service Desk Team permissions" },
    ...
    commands: ["npm run seed:jira", "npm run jira:scan", "npm run db:write"],
    docsPath: "/docs#jira",
  },
  "salesforce": {
    id: "salesforce",
    ...
    logoBg: "#00A1E0",
    logoColor: "#fff",
    tagline: "Users, integration accounts, connected apps — CRM identity surface.",
    setupTime: "10 min",
    what: [
      "All Salesforce users — human, integration, and automated process accounts",
      "Connected Apps (OAuth clients with org-wide access)",
      "Profile and permission set assignments",
      "Integration users used by middleware and ETL pipelines",
      "Einstein AI agent identities and Agentforce accounts",
    ],
    finds: [
      { sev: "critical", label: "Integration user with System Administrator profile" },
      { sev: "critical", label: "Connected App with full org access, no IP restriction" },
      { sev: "high",     label: "Inactive user with Salesforce admin profile (60+ days)" },
      { sev: "high",     label: "Integration user last active 90+ days — orphaned" },
      { sev: "medium",   label: "Connected App with refresh_token scope — long-lived" },
    ],
    fields: [
      { key: "SALESFORCE_INSTANCE_URL",  label: "Instance URL",     placeholder: "https://yourcompany.my.salesforce.com",
        hint: "Your Salesforce org URL — found in Setup → Company Settings → My Domain" },
      { key: "SALESFORCE_CLIENT_ID",     label: "Client ID",        placeholder: "3MVG9...",
        hint: "Setup → App Manager → New Connected App → Enable OAuth → Consumer Key" },
      { key: "SALESFORCE_CLIENT_SECRET", label: "Client Secret",    placeholder: "...", secret: true,
        hint: "Same Connected App → Consumer Secret. Scopes needed: api, refresh_token, full" },
      { key: "SALESFORCE_USERNAME",      label: "Username",         placeholder: "scanner@yourcompany.com",
        hint: "A dedicated integration user — assign a read-only profile minimum" },
      { key: "SALESFORCE_PASSWORD",      label: "Password + Token", placeholder: "Password123SecurityToken456", secret: true,
        hint: "Concatenate password + security token (no space). Get token: My Settings → Personal → Reset My Security Token" },
    ],
    envKeys: ["SALESFORCE_INSTANCE_URL", "SALESFORCE_CLIENT_ID", "SALESFORCE_CLIENT_SECRET", "SALESFORCE_USERNAME", "SALESFORCE_PASSWORD"],
    commands: ["npm run salesforce:scan", "npm run db:write"],
    docsPath: "/docs#salesforce",
  },
  "snowflake": {
    id: "snowflake",
    name: "Snowflake",
    ...
    logoBg: "#29B5E8",
    logoColor: "#fff",
    tagline: "Data warehouse user governance — service accounts, ETL identities, admin roles.",
    setupTime: "10 min",
    what: [
      "All Snowflake users — humans, service accounts, ETL pipeline users",
      "Role assignments (ACCOUNTADMIN, SYSADMIN, SECURITYADMIN)",
      "Service account detection — dbt, Fivetran, Airflow, Looker, Tableau",
      "Inactive accounts with ACCOUNTADMIN — highest privilege",
      "Users with RSA key pair only — API-only service accounts",
    ],
    finds: [
      { sev: "critical", label: "ACCOUNTADMIN user inactive 60+ days" },
      { sev: "critical", label: "Service account with ACCOUNTADMIN role" },
      { sev: "high",     label: "ETL user with SYSADMIN — far broader than needed" },
      { sev: "high",     label: "User account with no last login — possibly abandoned" },
      { sev: "medium",   label: "BI tool user (Looker, Tableau) with write permissions" },
    ],
    fields: [
      { key: "SNOWFLAKE_ACCOUNT",  label: "Account Identifier", placeholder: "xy12345.us-east-1",
        hint: "Admin → Account → Locator. Format: xy12345.us-east-1 (or xy12345 for US-1 legacy)" },
      { key: "SNOWFLAKE_USER",     label: "Scanner Username",   placeholder: "UNKOV_SCANNER",
        hint: "CREATE USER UNKOV_SCANNER PASSWORD=\'...' DEFAULT_ROLE=SECURITYADMIN; GRANT ROLE SECURITYADMIN TO USER UNKOV_SCANNER;" },
      { key: "SNOWFLAKE_PASSWORD", label: "Password",           placeholder: "...", secret: true,
        hint: "Password for the scanner user. For production use key-pair auth via SNOWFLAKE_PRIVATE_KEY instead." },
      { key: "SNOWFLAKE_ROLE",     label: "Role (optional)",    placeholder: "SECURITYADMIN",
        hint: "Role used for scanning — SECURITYADMIN minimum required to run SHOW USERS." },
    ],
    envKeys: ["SNOWFLAKE_ACCOUNT", "SNOWFLAKE_USER", "SNOWFLAKE_PASSWORD"],
    commands: ["npm run snowflake:scan", "npm run db:write"],
    docsPath: "/docs#snowflake",
  },
  "crowdstrike": {
    id: "crowdstrike",
    ...
    logoBg: "#E0001B",
    logoColor: "#fff",
    tagline: "Falcon console users, API clients — endpoint + identity correlation.",
    setupTime: "5 min",
    what: [
      "All Falcon console users and their assigned roles",
      "API clients (OAuth2 clients with sensor-wide data access)",
      "Dormant Falcon admin accounts (60+ days inactive)",
      "API clients with broad host data scope — no expiry",
      "Charlotte AI and detection automation identities",
    ],
    finds: [
      { sev: "critical", label: "Falcon Admin account inactive 60+ days" },
      { sev: "critical", label: "API client with write scope — no expiry set" },
      { sev: "high",     label: "API client last used 90+ days — potentially orphaned" },
      { sev: "high",     label: "Falcon admin with no MFA enrolled" },
      { sev: "medium",   label: "API client name matches no known owner" },
    ],
    fields: [
      { key: "CROWDSTRIKE_CLIENT_ID",     label: "API Client ID",       placeholder: "abcdef1234567890abcdef1234567890",
        hint: "Falcon Console → Support → API Clients & Keys → Add new API client → Client ID" },
      { key: "CROWDSTRIKE_CLIENT_SECRET", label: "API Client Secret",   placeholder: "...", secret: true,
        hint: "Same API client → Client Secret. Scopes: User Management (read), Hosts (read)" },
      { key: "CROWDSTRIKE_BASE_URL",      label: "Base URL (optional)", placeholder: "https://api.crowdstrike.com",
        hint: "US-1 default. EU-1: api.eu-1.crowdstrike.com · US-2: api.us-2.crowdstrike.com" },
    ],
    envKeys: ["CROWDSTRIKE_CLIENT_ID", "CROWDSTRIKE_CLIENT_SECRET"],
    commands: ["npm run crowdstrike:scan", "npm run db:write"],
    docsPath: "/docs#crowdstrike",
  },
};
...
const INTEGRATIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Snowflake",          id: "snowflake", status: "live",    desc: "Data warehouse user governance — ETL service accounts, ACCOUNTADMIN role sprawl, pipeline identity drift." },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Salesforce",                  id: "salesforce", status: "live",    desc: "Users, integration accounts, Connected Apps, Einstein AI identities. CRM identity surface." },
      { name: "Salesforce Einstein",         id: "salesforce", status: "live",    desc: "Einstein AI agent governance via Salesforce connector. CRM data access scope, user delegation." },
    ],
  },
  {
    cat: "SIEM & Security",
    ...
    items: [
      ...
      { name: "CrowdStrike Falcon", id: "crowdstrike", status: "live",    desc: "Falcon console users, API clients — endpoint + identity correlation. Admin account governance." },

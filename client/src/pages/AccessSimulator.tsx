    ...
    <div className="min-h-screen" style={{ backgroundColor: "#0a0f1e", color: "#e2e8f0" }}>
      ...
      <div style={{ paddingTop: 60, minHeight: "100vh" }}>
        <div style={{ padding: "1.25rem 2rem", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
          <div>
            <div style={{ fontSize: "1rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: "0.25rem" }}>Interactive Demo</div>
            <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#f1f5f9", marginBottom: "0.25rem" }}>Access Request Simulator</h1>
            <p style={{ color: "#64748b", fontSize: "1.0625rem" }}>Submit any access request and watch the Intent Engine decide in real-time. Fully explainable AI.</p>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 0 }}>
          ...
          <div style={{ padding: "2rem", borderRight: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              <div style={{ fontSize: "1.0625rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "0.75rem" }}>Quick Presets</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                {PRESETS.map(p => (
                  <button key={p.label} onClick={() => applyPreset(p)}
                    style={{ padding: "0.375rem 0.875rem", borderRadius: 9999, fontSize: "1.0625rem", border: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(255,255,255,0.04)", color: "#94a3b8", cursor: "pointer", transition: "all 0.15s" }}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
              {[
                ...
                <div key={f.label}>
                  <label style={{ fontSize: "1rem", color: "#64748b", textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: "0.5rem" }}>{f.label}</label>
                  <select value={f.val} onChange={e => f.set(e.target.value)}
                    style={{ width: "100%", padding: "0.625rem 0.875rem", backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#e2e8f0", fontSize: "1.0625rem", outline: "none", cursor: "pointer" }}>
            ...
            <div style={{ marginBottom: "1.25rem" }}>
              <label style={{ fontSize: "1rem", color: "#64748b", textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: "0.5rem" }}>Business Justification</label>
              <textarea value={explanation} onChange={e => setExplanation(e.target.value)} placeholder="Describe why this access is needed..." rows={3}
                style={{ width: "100%", padding: "0.75rem", backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#e2e8f0", fontSize: "1.0625rem", resize: "vertical", outline: "none", fontFamily: "inherit" }} />
            </div>

            <div style={{ display: "flex", gap: "0.75rem" }}>
              <button onClick={runSim} disabled={simState === "running"}
                style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.75rem 1.75rem", backgroundColor: simState === "running" ? "rgba(0,97,212,0.3)" : "#0061d4", border: "none", borderRadius: 10, color: "#fff", fontWeight: 700, fontSize: "1rem", cursor: simState === "running" ? "not-allowed" : "pointer", transition: "all 0.15s" }}>
                <Play style={{ width: 16, height: 16 }} />
                {simState === "running" ? "Processing..." : "Run Simulation"}
              </button>
              <button onClick={reset}
                style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.75rem 1.25rem", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, color: "#94a3b8", fontWeight: 600, fontSize: "1.0625rem", cursor: "pointer" }}>
            ...
            {simState === "running" && (
              <div style={{ marginTop: "1.5rem", padding: "1.25rem", backgroundColor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12 }}>
                <div style={{ fontSize: "1.0625rem", color: "#60a5fa", marginBottom: "0.75rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            {result && simState === "done" && (
              <div style={{ marginTop: "1.5rem", padding: "1.5rem", backgroundColor: DECISION_STYLES[result.decision].bg, border: `1px solid ${DECISION_STYLES[result.decision].border}`, borderRadius: 14 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" }}>
                  <div>
                    <div style={{ fontSize: "1.0625rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "0.25rem" }}>AI Decision</div>
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <div style={{ fontSize: "1.0625rem", color: "#475569", textTransform: "uppercase" }}>Confidence</div>
                    <div style={{ fontSize: "1.0625rem", color: "#64748b", marginTop: "0.25rem" }}>{result.time_ms}ms decision time</div>
                  </div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {result.reasons.map((r, i) => (
                    <div key={i} style={{ display: "flex", gap: "0.625rem", alignItems: "flex-start" }}>
                      ...
                      <div style={{ fontSize: "1.0625rem", color: "#94a3b8" }}>{r}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={{ padding: "1.5rem" }}>
            <div style={{ fontSize: "1.0625rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "1rem" }}>Simulation History</div>
            {history.length === 0 && (
              <div style={{ fontSize: "1rem", color: "#374151", fontStyle: "italic" }}>Run your first simulation →</div>
            )}
            {history.map((h, i) => {
              ...
              return (
                <div key={i} style={{ padding: "0.75rem", backgroundColor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8, marginBottom: "0.5rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.25rem" }}>
                    <span style={{ fontSize: "1rem", color: "#64748b" }}>{h.identityType}</span>
                    <span style={{ fontSize: "1.0625rem", fontWeight: 700, color: s.text }}>{h.decision}</span>
                  </div>
                  <div style={{ fontSize: "1.0625rem", color: "#94a3b8" }}>→ {h.resource}</div>
                  <div style={{ fontSize: "1.0625rem", color: "#475569", marginTop: "0.25rem" }}>{h.confidence}% confidence · {h.time_ms}ms</div>
                </div>
              );
            })}
            ...
            <div style={{ marginTop: "2rem", padding: "1rem", backgroundColor: "rgba(0,97,212,0.05)", border: "1px solid rgba(0,97,212,0.15)", borderRadius: 10 }}>
              <div style={{ fontSize: "1rem", color: "#60a5fa", fontWeight: 700, marginBottom: "0.5rem" }}>💡 KEY INSIGHT</div>
              <div style={{ fontSize: "1.0625rem", color: "#64748b", lineHeight: 1.5 }}>Unkov processes 1,200+ access requests/day automatically. Only 3% require human review. Legacy tools require 100% manual review.</div>
            </div>
          </div>
        </div>
      </div>

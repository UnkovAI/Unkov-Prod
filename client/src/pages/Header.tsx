          ...
          <div className="hidden md:flex" style={{ alignItems: "center", gap: "0.25rem", flexShrink: 0 }}>
            {user ? (
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.25rem 0.5rem 0.25rem 0.25rem", borderRadius: "9999px", border: "1px solid #dcd6ce", backgroundColor: "transparent", cursor: "pointer", transition: "background-color 0.12s, border-color 0.12s" }}
                ...
                <div style={{ width: 24, height: 24, borderRadius: "50%", backgroundColor: "#00297a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.65rem", fontWeight: 700, color: "#ffffff", flexShrink: 0 }}>
                  {user.name && user.name !== user.email
                    ? user.name.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2)
                    : user.email.slice(0, 2).toUpperCase()}
                </div>
                <span style={{ fontSize: "0.8125rem", fontWeight: 500, color: "#3d3d52", maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
